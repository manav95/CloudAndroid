    ...
    private AsyncTask<String,Void,ArrayList<Award>> databaseTask = new AsyncTask<String, Void, ArrayList<Award>>() {
        ...
        protected ArrayList<Award> doInBackground(String... params) {
            ...
            try {
                ...
                type = params[0];
                switch (params[0]) {
                    case "Zip": awardSql = "select a.category, a.type_description, a.description, a.create_date, recipient.recipient_name, a.total_obligation from awards a LEFT JOIN legal_entity recipient ON recipient.legal_entity_id = a.recipient_id WHERE (a.recipient_id in (select location_id from references_location l where l.zip5 = ?) or a.place_of_performance_id in (select location_id from references_location l where l.zip5 = ? )) ORDER BY a.create_date DESC LIMIT 15";
                                ps = con.prepareStatement(awardSql);
                                ps.setString(1, params[1]);
                                ps.setString(2, params[1]);
                                break;
                    case "City": awardSql = "select a.category, a.type_description, a.description, a.create_date, recipient.recipient_name, a.total_obligation from awards a LEFT JOIN legal_entity recipient ON recipient.legal_entity_id = a.recipient_id WHERE (a.recipient_id in (select location_id from references_location l where l.city_name = ?) or a.place_of_performance_id in (select location_id from references_location l where l.city_name = ? )) ORDER BY a.create_date DESC LIMIT 15";
                                 ps = con.prepareStatement(awardSql);
                                 ps.setString(1, params[1].toUpperCase());
                                 ps.setString(2, params[1].toUpperCase());
                                 break;
                    case "County": awardSql = "select a.category, a.type_description, a.description, a.create_date, recipient.recipient_name, a.total_obligation from awards a LEFT JOIN legal_entity recipient ON recipient.legal_entity_id = a.recipient_id WHERE (a.recipient_id in (select location_id from references_location l where l.county_name = ?) or a.place_of_performance_id in (select location_id from references_location l where l.county_name = ? )) ORDER BY a.create_date DESC LIMIT 15";
                                   ps = con.prepareStatement(awardSql);
                                   String county = params[1].split("\\s+")[0];
                                   ps.setString(1, county.toUpperCase());
                                   ps.setString(2, county.toUpperCase());
                                   break;
                    case "State": awardSql = "select a.category, a.type_description, a.description, a.create_date, recipient.recipient_name, a.total_obligation from awards a LEFT JOIN legal_entity recipient ON recipient.legal_entity_id = a.recipient_id WHERE (a.recipient_id in (select location_id from references_location l where l.state_name = ?) or a.place_of_performance_id in (select location_id from references_location l where l.state_name = ? )) ORDER BY a.create_date DESC LIMIT 15";
                                  ps = con.prepareStatement(awardSql);
                                  ps.setString(1, params[1].toUpperCase());
                                  ps.setString(2, params[1].toUpperCase());
                                  break;
                    default:      awardSql = "select a.category, a.type_description, a.description, a.create_date, recipient.recipient_name, a.total_obligation from awards a LEFT JOIN legal_entity recipient ON recipient.legal_entity_id = a.recipient_id WHERE (a.recipient_id in (select location_id from references_location l where l.congressional_code = ? and l.state_code = ?) or a.place_of_performance_id in (select location_id from references_location l where l.congressional_code = ? AND l.state_code= ?)) ORDER BY a.create_date DESC LIMIT 15";
                                  ps = con.prepareStatement(awardSql);
